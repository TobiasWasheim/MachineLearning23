...
class LogisticRegressionClassifier():
    ...
    def predict(self, X):
        ...
        """
        prediction = [logistic(np.dot(self.w,xi)) for xi in X]
        out = [1 if i>=.5 else -1 for i in prediction]
        return out
    ...
    def score(self, X, y):
        ...
        for i in range(len(y)):
            if predictions[i] == y[i]: scores[i] = 1
            else: scores[i] = 0
        s = np.sum(scores)/len(y)
        return s
