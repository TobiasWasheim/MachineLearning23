...
def softmax(X):
    ...
    for i in range(len(X)):
        n = X[i,:]
        nmax = np.amax(n)
        denominator = np.log(np.sum([np.exp(d-nmax) for d in n])) + nmax
        for j in range(len(n)):
            res[i,j] = np.exp(X[i,j] - denominator)
    return res
...
def relu(x):
    ...
    """
    # res = [np.maximum(0,i) for i in x]
    res = np.maximum(np.zeros(shape=x.shape),x)
    return res
...
class NetClassifier():
    ...
    def predict(self, X, params=None):
        ...
        if params is None:
            params = self.params
        pred = None
        hidden_layer_in = np.dot(X,params.get("W1"))+params.get("b1")
        hidden_layer_out = relu(hidden_layer_in)
        out = np.dot(hidden_layer_out,params.get("W2"))+params.get("b2")
    ...
    def score(self, X, y, params=None):
        ...
        acc=sum([int(y[i]==prediction[i]) for i in range(len(y))])/len(y)
        return acc
    ...
    def cost_grad(X, y, params, c=0.0):
        ...
        d_b1 = None
        d_b2 = None
        y_in_k = one_in_k_encoding(y, W2.shape[1]) # shape n x k


        # forward pass:
        hidden_layer_in = X@W1+b1
        hidden_layer_out = relu(hidden_layer_in)
        out = hidden_layer_out@W2+b2
        decay = c*(np.sum(np.sum(W1**2))+np.sum(np.sum(W2**2)))
        cost = -np.log(softmax(out))*y_in_k
        cost_out = np.sum(cost)/len(cost) + decay

        # backward pass:
        d_out = softmax(out)-y_in_k

        relu_indicator = np.array(hidden_layer_out,copy=True,dtype=bool)
        relu_indicator = np.array(relu_indicator,dtype=int)

        d_w1 = (np.multiply((d_out@W2.T),relu_indicator).T@X).T/len(X) + c*2*W1
        d_b1 = (np.multiply((d_out@W2.T),relu_indicator).T@np.ones((len(X),1))).T / len(X) # normalize because we sum over datapoints
        d_w2 = (d_out.T@hidden_layer_out).T/len(X) + c*2*W2
        d_b2 = (d_out.T@np.ones((len(X),1))).T / len(X) # normalize because we sum over datapoints
        
        # the return signature
        return cost_out, {'d_w1': d_w1, 'd_w2': d_w2, 'd_b1': d_b1, 'd_b2': d_b2}
    ...
    def fit(self, X_train, y_train, X_val, y_val, init_params, batch_size=32, lr=0.1, c=1e-4, epochs=30):
        ...
        param_hist = make_dict([],[],[],[])
        
        
        for epoch in range(epochs):
            random_sequence = list(range(len(X_train)))
            np.random.shuffle(random_sequence) # make random sequence to shuffle training data
            x_data, y_data = X_train[random_sequence], y_train[random_sequence] # make shuffled training data
            batch_start, batch_end = 0, batch_size # used to cycle through data, batch_start/_end is used as indices
            while batch_end < len(x_data):
                x_batch, y_batch = x_data[batch_start:batch_end],y_data[batch_start:batch_end]
                cost, grad = self.cost_grad(x_batch, y_batch, make_dict(W1,b1,W2,b2),c)
                W1 -= lr*grad['d_w1']
                W2 -= lr*grad['d_w2']
                b1 -= lr*grad['d_b1']
                b2 -= lr*grad['d_b2']
            ...
            val_loss, _ = self.cost_grad(X_val,y_val,make_dict(W1,b1,W2,b2),c)
            val_acc = self.score(X_val,y_val,make_dict(W1,b1,W2,b2))
            train_acc = self.score(X_train,y_train,make_dict(W1,b1,W2,b2))
            for k,v in make_dict(W1,b1,W2,b2).items():
                param_hist.get(k).append(v)
            for k,v in zip(['train_loss','train_acc','val_loss','val_acc'],[cost,train_acc,val_loss,val_acc]):
                hist.get(k).append(v)

        best_fit = np.argmin(hist.get('val_acc'))
        W1 = param_hist.get('W1')[best_fit]
        W2 = param_hist.get('W2')[best_fit]
        b1 = param_hist.get('b1')[best_fit]
        b2 = param_hist.get('b2')[best_fit]
...
def numerical_grad_check(f, x, key):
    ...
    while not it.finished:    
        ...
        # print('numerical gradient error index {0}, numerical gradient {1}, computed gradient {2}'.format(dim, num_grad, grad[dim]))
        it.iternext()
